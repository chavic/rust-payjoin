...
DOTNET_8_SDK_VERSION=$(dotnet --list-sdks | awk '/^8\./ { print $1; exit }')
if [[ -n $DOTNET_8_SDK_VERSION ]]; then
    cat > "$WORKDIR/global.json" <<EOF
{
  "sdk": {
    "version": "$DOTNET_8_SDK_VERSION",
...
dotnet new console --framework net8.0 --output "PayjoinSmoke" --no-restore
...
if [[ -n $RID ]]; then
    ...
    if [[ -z $(find "PayjoinSmoke/bin/Debug/net8.0" -name "$LIBNAME" -print -quit) ]]; then
        echo "Expected native asset $LIBNAME was not copied for $RID" >&2
        exit 1
    fi
fi

dotnet "PayjoinSmoke/bin/Debug/net8.0/PayjoinSmoke.dll"
