...
use crate::send::error::{
    PrimitiveInputError, SenderPersistedError, SenderPrimitiveError, SenderReplayError,
};
use crate::uri::PjUri;
...
impl SenderBuilder {
    ...
    #[uniffi::constructor]
    pub fn new(psbt: String, uri: Arc<PjUri>) -> Result<Self, SenderPrimitiveError> {
        let psbt = payjoin::bitcoin::psbt::Psbt::from_str(psbt.as_str())
            .map_err(PrimitiveInputError::from)
            .map_err(SenderPrimitiveError::Input)?;
    ...
    pub fn build_recommended(
        &self,
        min_fee_rate: u64,
    ) -> Result<InitialSendTransition, SenderPrimitiveError> {
        self.0
            .clone()
            ...
            .map_err(|e: payjoin::send::BuildSenderError| SenderPrimitiveError::Build(Arc::new(e.into())))
    }
