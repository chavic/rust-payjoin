...
describe("Async Persistence tests", () => {
    test("receiver async persistence", async () => {
        ...
        const transition = builder.build();

        // Use saveAsync instead of save
        await transition.saveAsync(persister);

        assert.strictEqual(persister.events.length, 1, "Should have one event");
        assert.strictEqual(persister.closed, false, "Should not be closed");
    ...
    test("sender async persistence", async () => {
        ...
        const receiverPersister = new InMemoryReceiverPersister(1);
        const address = "2MuyMrZHkbHbfjudmKUy45dU4P17pjG2szK";
        const ohttpKeys = payjoin.OhttpKeys.decode(
            new Uint8Array([
                0x01, 0x00, 0x16, 0x04, 0xba, 0x48, 0xc4, 0x9c, 0x3d, 0x4a,
        ...
        const psbt =
            "";
        const transition = new payjoin.SenderBuilder(
            psbt,
            uri,
        ).buildRecommended(BigInt(1000));

        // Use saveAsync instead of save
        const withReplyKey = await transition.saveAsync(senderPersister);

        assert.strictEqual(
            senderPersister.events.length,
            1,
            "Should have one event",
        );
        assert.strictEqual(
            senderPersister.closed,
            false,
